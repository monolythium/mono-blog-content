...
The short version: a Rust/RISC-V-native settlement layer for the autonomous economy, with deterministic finality, post-quantum signatures at the wire, operator clusters that work like a marketplace rather than a stake-weighted auction, an encrypted-by-default mempool, native MRC asset standards, native agent-commerce primitives, and a hardware sovereignty story we genuinely believe is the right floor for institutional-grade nodes. The public testnet is opening, the browser wallet and explorer are public, and the signed desktop/operator release tracks are gated by the evidence described below. Fifteen repositories are public today. The whitepaper is published. The docs site is open. The Genesis Liquidity Program is running.
...
Hot paths — token standards, NFT standards, multi-token, vault, spot order book, bridge verification, name registry, agent-commerce primitives, encrypted-mempool flow — live as audited **native modules** rather than as user-deployed contracts. Native modules sit behind precompile addresses and use the same calling conventions as user contracts; the difference is that the protocol owns and audits them.
...
EVM liquidity reaches Monolythium through bridges, not through native execution. The chain accepts Ethereum-tooling-compatible RPC reads (`eth_chainId`, `eth_blockNumber`) so that exchanges with existing integrations can list LYTH without retooling; the actual settlement happens in the native lane.
...
## Privacy: bifurcated denomination, encrypted mempool
...
External liquidity reaches Monolythium through bridge routes, cross-chain swaps, and issuer-supported integrations rather than through native compatibility execution. The public bridge model is deliberately vendor-neutral: zero-knowledge or light-client verification where feasible, explicit trust-disclosure metadata where a route uses a different verifier model, and protocol floors that every route must satisfy before users or agents rely on it. When a route is enabled, its verifier model, drain cap, circuit breaker state, insurance pool, and last incident date are on-chain and visible to every wallet and indexer before the user signs.

Seven consensus-enforced safety floors apply to any bridge route that registers: drain caps, circuit breakers, no post-deploy admin-key config path, trust-disclosure metadata, route registration refusal when floors fail, slashable insurance pool, and SDK intents-native route selection. The floors come out of a careful read of historical bridge incidents across the industry — we treat those incidents as shared lessons rather than as competitive material, and the safety surface is designed so the same floors apply uniformly to every bridge that integrates.

The route-cooldown framework is bridge-agnostic. It is parameterized per route by finality, verifier maturity, drain cap, circuit-breaker posture, and audit evidence. None of this is a comment on any single provider; the public promise is that route risk is explicit and bounded at the protocol layer.
...
- **Public testnet `69420`** — a foundation-operated operator cluster, four-second deterministic finality, ML-DSA-65 at the wire, optional opt-in encrypted mempool. The TLS-fronted public RPC is at `rpc.monolythium.com`. Chain identity, peers, and explorer endpoints are in the [`chain-registry`](https://github.com/monolythium/chain-registry).
...
- **Mono Core SDK** — [`mono-core-sdk`](https://github.com/monolythium/mono-core-sdk) in Rust and TypeScript, generated from the same Rust types the chain itself uses. Address helpers, ML-DSA-65 signing, fee formatting (`formatLyth`/`parseLyth` with 8 decimals; the atomic unit is "lythoshi"), MRC token helpers, RPC client, bridge-route disclosure, REST `/api/v1` client, Mesh-interop helpers. The crate ships on crates.io and the TypeScript package ships on npm.
...
The genesis supply is **100,000,000 LYTH**, fixed at genesis. Annual reward issuance is capped at **8%**. Base fees burn. The treasury draws from a genesis reserve rather than an inflation tap; there is no holder-adjustable inflation lever. LYTH precision is **8 decimals**, with the atomic unit named "lythoshi" — `formatLyth` and `parseLyth` from the SDK handle the conversion. Do not use 18-decimal helpers from EVM tooling; they silently mis-render LYTH balances by 10^10.
